document.addEventListener("DOMContentLoaded", function () {
  ...
  const tablaDtotalContainer = document.getElementById(
    "tabla-dtotal-container"
  );
  const tablaDtotal = document.getElementById("tabla-dtotal");
  const calculosFinales = document.getElementById("calculos-finales");

  const tablaRetiroContainer = document.getElementById(
    "tabla-retiro-container"
  );
  const tablaRetiro = document.getElementById("tabla-retiro");
  const calculosFinalesRetiro = document.getElementById(
    "calculos-finales-retiro"
  );
  ...
  const tablaFideicomisoContainer = document.getElementById(
    "tabla-fideicomiso-container"
  );
  const tablaFideicomiso = document.getElementById("tabla-fideicomiso");
  const calculosFinalesFideicomiso = document.getElementById(
    "calculos-finales-fideicomiso"
  );

  const tablaSeguroVidaContainer = document.getElementById(
    "tabla-segurovida-container"
  );
  const tablaSeguroVida = document.getElementById("tabla-segurovida");
  const calculosFinalesSeguroVida = document.getElementById(
    "calculos-finales-segurovida"
  );

  const campoEdadContratante = document.getElementById(
    "campo-edad-contratante"
  );
  ...
  const planesRetiro = {
    ...
    RETIRO_30_65: { porcentajeBase: 0.06 },
    RETIRO_30_70: { porcentajeBase: 0.0528 },
    RETIRO_40_55: { porcentajeBase: 0.1 },
    RETIRO_40_60: { porcentajeBase: 0.085 },
    RETIRO_40_65: { porcentajeBase: 0.09 },
    RETIRO_40_70: { porcentajeBase: 0.0845 },
    RETIRO_50_60: { porcentajeBase: 0.11 },
    RETIRO_50_65: { porcentajeBase: 0.12 },
  ...
  function generarTablaDotal() {
    ...
    for (let i = 0; i < 10; i++) {
      if (i < 2) {
        ...
        const multiplicadores = [
          0.414, 0.5521, 0.6901, 0.8281, 0.9621, 1.1, 1.2381, 1.3761,
        ];
        let sumaAcumulada = inversiones
          .slice(0, i + 1)
          .reduce((a, b) => a + b, 0);
    ...
    recuperacionTotalSpan.textContent =
      formatoMoneda(recuperacionTotal) + " Pesos";
    ...
    if (selectedPlan === "VIDA MUJER") {
      mensajeBeneficios.textContent =
        "Beneficios adicionales: Invalidez total y permanente, exención de pago de primas por invalidez, perdidas orgánicas y beneficio de muerte accidental, protección por cáncer femenino, protección para complicaciones del embarazo.";
    } else {
      mensajeBeneficios.textContent =
        "Beneficios adicionales: Invalidez total y permanente, exención de pago de primas por invalidez, perdidas orgánicas y beneficio de muerte accidental.";
    }
  ...
  function generarTablaRetiroGenerica(config) {
    const { edad, inversion, filas, porcentajeBase, multiplicadoresDotal } =
      config;
    ...
    for (let i = 0; i < filas; i++) {
      if (i < 2) {
        ...
        multiplicadoresDotal &&
        multiplicadoresDotal[i - 2] !== undefined
      ) {
    ...
    for (let i = 0; i < filas; i++) {
      if (i < 2) {
        ...
        inversionAcumulada.push(
          inversiones[0] + inversiones[1] + inversiones[2]
        );
      } else {
    ...
    document.getElementById("inversion-total-retiro").textContent =
      formatoMoneda(inversionTotal) + " Pesos";
    document.getElementById("recuperacion-total-retiro").textContent =
      formatoMoneda(recuperacionTotalFinal) + " Pesos";
  ...
  function generarTablaVidaMujer() {
    ...
    document.getElementById("inversion-total-vida").textContent =
      formatoMoneda(inversionTotal) + " Pesos";
    document.getElementById("recuperacion-total-vida").textContent =
      formatoMoneda(recuperacionTotalFinal) + " Pesos";
  ...
  function generarTablaFideicomisoEdu(config) {
    ...
    for (let i = 0; i < filas; i++) {
      if (i < 2) {
        ...
        tasaRecuperacion.push(tasaRecuperacion[i - 1] + 0.1);
      }
    }
    ...
    document.getElementById("inversion-total-fideicomiso").textContent =
      formatoMoneda(inversionTotal) + " Pesos";
    document.getElementById("recuperacion-total-fideicomiso").textContent =
      formatoMoneda(recuperacionTotalFinal) + " Pesos";
    document.getElementById("calculos-finales-fideicomiso").style.display =
      "flex";
  ...
  function generarTablaSeguroVida() {
    ...
    if (inversion < 30000) {
      alert(
        "La inversión mínima para el plan SEGURO DE VIDA es de 30,000 pesos."
      );
      return;
    }
    ...
    if (edad <= 0 || edad < 20 || edad > 60) {
      alert(
        "La edad para el plan SEGURO DE VIDA debe estar entre 20 y 60 años."
      );
      return;
    }
    ...
    document.getElementById("inversion-total-segurovida").textContent =
      formatoMoneda(totalInversion) + " Pesos";
    document.getElementById("suma-total-segurovida").textContent =
      formatoMoneda(totalSuma) + " Pesos";
    document.getElementById("calculos-finales-segurovida").style.display =
      "flex";
  ...
  btnGenerar.addEventListener("click", function () {
    ...
    if (!nombreReq || !correoReq || !telefonoReq) {
      alert(
        "Por favor complete Nombre, Correo y Teléfono antes de solicitar la cotización."
      );
      return;
    }
    ...
    if (selectedPlan === "VIDA MUJER") {
      if (edad < 18 || edad > 50) {
        alert(
          "Para el plan VIDA MUJER, la edad debe estar entre 18 y 50 años."
        );
        return;
      }
    } else if (
      ["DOTAL 10", "RETIRO", "SEGURO DE VIDA"].includes(selectedPlan)
    ) {
      if (edad < 20 || edad > 60) {
    ...
    if (selectedPlan === "DOTAL 10") {
      ...
      const edadContratante =
        parseInt(document.getElementById("edad-contratante").value) || 0;
      const edadMenor =
        parseInt(document.getElementById("edad-menor").value) || 0;
      const inversion = parseFloat(limpiarFormato(inputInversion.value)) || 0;

      if (inversion < 36000) {
        alert(
          "La inversión mínima para el Fideicomiso Educativo es de 36,000 pesos."
        );
        return;
      }
  ...
  // --------------------------------------------------
  // Formspree: enviar datos del solicitante al propietario
  // --------------------------------------------------
  ...
  // Correo del propietario que debe recibir la notificación
  const OWNER_EMAIL = "hola@desingsgdl.mx";

  // Opcional: endpoint que acepte un PDF (base64) y lo envíe por email.
  // Por defecto intentará usar el endpoint local '/send-email' si el valor está vacío.
  // Cámbialo por la URL pública de tu servidor en producción si es necesario.
  // Aquí apuntamos al staging/producción proporcionado: https://lpsmx.com/cotizadorlps/
  const SERVERLESS_ENDPOINT = "https://lpsmx.com/cotizadorlps/send-email"; // cambiar si la ruta difiere
  ...
  async function enviarDatosAlOwner() {
    ...
    if (!FORMSPREE_ENDPOINT || FORMSPREE_ENDPOINT.includes("XXXXXXXX")) {
      console.warn(
        "Formspree endpoint no configurado. Edita FORMSPREE_ENDPOINT en js/script.js"
      );
      return;
    }
    ...
    const inversionTotal =
      document.getElementById("inversion-total")?.textContent ||
      document.getElementById("inversion-total-retiro")?.textContent ||
      document.getElementById("inversion-total-vida")?.textContent ||
      "";
    const recuperacionTotal =
      document.getElementById("recuperacion-total")?.textContent ||
      document.getElementById("recuperacion-total-retiro")?.textContent ||
      document.getElementById("recuperacion-total-vida")?.textContent ||
      "";
    ...
    try {
      ...
      if (res.ok) {
        console.log("Datos enviados al propietario via Formspree");
  ...
  async function generarPDF() {
    ...
    title.textContent =
      document.getElementById("titulo-header")?.textContent || "Cotización";
    ...
    let tablaVisible = tablas.find(
      (t) =>
        t.offsetParent !== null || window.getComputedStyle(t).display !== "none"
    );
  ...
  // Generar un PDF resumen (pequeño) usando jsPDF directamente
  // Contendrá: Nombre, Correo, Plan, Inversión Total, Recuperación Total y Fecha
  function generarPDFResumen() {
    try {
      ...
      pdf.setFontSize(16);
      pdf.text("Resumen - Cotización LPS", margin, y);
      y += 30;
      ...
      pdf.text(`Nombre: ${nombre}`, margin, y);
      y += 18;
      pdf.text(`Correo: ${correo}`, margin, y);
      y += 18;
      pdf.text(`Plan: ${plan}`, margin, y);
      y += 18;
      pdf.text(`Inversión Total: ${invTotal}`, margin, y);
      y += 18;
      pdf.text(`Recuperación Total: ${recTotal}`, margin, y);
      y += 24;
      pdf.text(`Fecha: ${new Date().toLocaleString()}`, margin, y);

      // No forzamos descarga aquí; devolvemos el objeto jsPDF para obtener el blob
      return pdf;
    } catch (err) {
      console.error("Error generando PDF resumen:", err);
      return null;
    }
  }

  // Enviar el PDF (blob) a un endpoint serverless que haga el envío por correo
  // Devuelve true si el servidor respondió exitosamente
  async function enviarPdfAlServidor(pdfInputs, destinoEmail) {
    // Resolver endpoint: si no está configurado, usar /send-email (asumimos servidor local o proxy)
    const endpoint =
      SERVERLESS_ENDPOINT && SERVERLESS_ENDPOINT.trim() !== ""
        ? SERVERLESS_ENDPOINT
        : "/send-email";

    try {
      // pdfInputs puede ser: un Blob único, un objeto { blob, filename }, o un array de estos
      let items = [];
      if (!pdfInputs) items = [];
      else if (Array.isArray(pdfInputs)) items = pdfInputs;
      else items = [pdfInputs];

      const attachments = [];
      for (const it of items) {
        // permitir que se pase directamente un Blob o un objeto con { blob, filename }
        const blob = it.blob || it;
        const filename = it.filename || "cotizacion_lps.pdf";
      ...
      const payload = {
        ...
        message:
          "Adjunto encontrarás tu(s) cotización(es) generada(s) desde la web.",
        attachments,
      };
  ...
  // Primero generamos el PDF (html2canvas + jsPDF), luego intentamos usar la API Web Share con archivos.
  // Si no está disponible, forzamos la descarga y abrimos mailto como alternativa.
  async function enviarPorCorreo() {
    ...
    // Generar ambos PDFs: detalle (html2canvas) y resumen (jsPDF)
    const pdfDetalle = await generarPDF();
    if (!pdfDetalle) return;
    const pdfResumen = generarPDFResumen();
    // obtener blobs
    try {
      const pdfBlobDetalle = pdfDetalle.output("blob");
      const pdfBlobResumen = pdfResumen ? pdfResumen.output("blob") : null;

      // 1) Intentar enviar ambos al servidor si está disponible (POST /send-email o URL configurada)
      const inputs = [
        { blob: pdfBlobDetalle, filename: "cotizacion_lps_detalle.pdf" },
      ];
      if (pdfBlobResumen)
        inputs.push({
          blob: pdfBlobResumen,
          filename: "cotizacion_lps_resumen.pdf",
        });

      const okServer = await enviarPdfAlServidor(inputs, correoDestino);
      if (okServer) return { shared: true, sentByServer: true };
      // si falla, continuar con las siguientes alternativas

      // 2) Si el navegador soporta la API Web Share con archivos, usarla (esto puede abrir la ventana de compartir en Windows/otros)
      const file = new File([pdfBlobDetalle], "cotizacion_lps_detalle.pdf", {
        type: "application/pdf",
      });
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        // incluir también el resumen si existe
        const filesToShare = pdfBlobResumen
          ? [
              file,
              new File([pdfBlobResumen], "cotizacion_lps_resumen.pdf", {
                type: "application/pdf",
              }),
            ]
          : [file];
        await navigator.share({
          files: filesToShare,
          title: "Cotización LPS",
          text: "Adjunto cotización.",
        });
        return { shared: true };
      }

      // 3) En escritorio o si la API Web Share no está disponible: forzar descarga del detalle y del resumen (si aplica) y abrir mailto prellenado al correo del usuario
      const url = URL.createObjectURL(pdfBlobDetalle);
      const a = document.createElement("a");
      a.href = url;
      a.download = "cotizacion_lps_detalle.pdf";
      ...
      if (pdfBlobResumen) {
        const url2 = URL.createObjectURL(pdfBlobResumen);
        const a2 = document.createElement("a");
        a2.href = url2;
        a2.download = "cotizacion_lps_resumen.pdf";
      ...
      bodyLines.push(
        "Adjunto encontrarás la cotización generada (se descargó automáticamente; por favor adjunta el PDF al correo si deseas enviarlo)."
      );
      bodyLines.push("");
  ...
  function calcularResultadosFinalesRetiro() {
    const inversionTotalSpan = document.getElementById(
      "inversion-total-retiro"
    );
    const recuperacionTotalSpan = document.getElementById(
      "recuperacion-total-retiro"
    );

    const inversionTotal = inversiones.reduce((a, b) => a + b, 0);
    const recuperacionTotal =
      recuperacionTotalArr[recuperacionTotalArr.length - 1]; // Última fila
